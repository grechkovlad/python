...
def get_latest_log_info(logdir):
    logfiles = [
        get_log_info(
            join(
                logdir,
                path)) for path in listdir(logdir) if is_log_file(
            join(
                logdir,
                path))]
    if (not logfiles):
    ...
    logfiles = sorted(
        logfiles,
        reverse=True,
        key=lambda log_info: log_info.date)
...
def median(sorted_values):
    ...
    if (arr_len % 2 == 0):
        return (sorted_values[arr_len // 2 - 1] +
                sorted_values[arr_len // 2]) / 2.0
...
def write_report(report_dir, report, log_date):
    try:
        ...
        if (not exists(report_dir)):
            logging.exception(
                'Error while creating directory for report: %s',
                report_dir)
    try:
